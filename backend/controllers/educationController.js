...
exports.getEducationContent = async (req, res) => {
  try {
    const { category, language = 'en', gender = 'all', ageGroup = 'all' } = req.query;
    ...
    let content = await Education.find(filter).sort({ category: 1, createdAt: 1 });
    ...
    content = content.map(item => ({
      ...
      description: language === 'rw' ? item.description.rw : item.description.en,
...
exports.addEducationContent = async (req, res) => {
  try {
    const { category, title, description, content, icon, type, videoUrl, targetAudience, ageGroup, source } = req.body;
